  ...
  yargs.command('track-ls', 'List currently running trackings', (yargs) => {}, () => {
    conf.getTrackers().then((trackings) => {
      console.log(trackings.reduce((str, track) => {
        let rt = track.recordedTime;
        let activeFlag = '⌛';

        if (!track.inactive) {
          rt += moment().diff(moment(track.started), 'seconds');
          activeFlag = '▶';
        }

        rt = Math.ceil(rt/60);
        return str + `${activeFlag} ${track.issue}\t${rt} min\t${track.comment}\n\r`;
      }, ''));
    }).catch((e) => {
      console.error(e);
      process.exit(1);
    ...
    conf.getOneTracker(argv.index).then((data) => {
      if (data.inactive) {
        data.inactive = false;
        data.started =  moment().format();
        console.log(`Tracker #${argv.index} unpaused.`);
      } else {
        data.inactive = true;
        data.recordedTime += moment().diff(moment(data.started), 'seconds');
        let inMin = moment().diff(moment(data.started), 'minutes');
        console.log(`Paused and saved ${inMin} minutes to tracker #${argv.index}`);
      }
